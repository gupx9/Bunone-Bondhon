...
export default async function ShopPage({ searchParams }: ShopPageProps) {
  ...
  const filteredProducts = searchTerm
    ? products.filter((product) => product.productName.toLowerCase().includes(searchTerm))
    : products;
  ...
  return (
    <div className="min-h-screen bg-warm-radial">
      ...
      <SectionShell
        ...
        description="Search product names instantly and browse the latest catalog from Prisma-backed products."
      >
        <div className="space-y-6">
          <ShopSearch initialValue={searchTerm} />
          ...
          {filteredProducts.length ? (
            ...
            <div className="surface p-8 text-center text-brown/70">No products match your search.</div>
          )}
        </div>
      </SectionShell>
    </div>
